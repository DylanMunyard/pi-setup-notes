...
- GCP LB connects to ratden.dman.dev:445
- ratden.dman.dev resolves to my static IP assigned by the ISP
...
HAProxy is configured to only accept SSL connections on 443 (inside the container), and serves it's own certificate signed by letsencrypt for `*.dman.dev`.


## Use a domain name
https://cloud.google.com/dns/docs/tutorials/create-domain-tutorial#set-up-domain \
__Relevant steps__
- Step 1 - Purchased dman.dev
...
Load balancer is the 'front end' for the Pi. Requests to dman.dev will 
...
E.g. if a request comes in for teamcity.dman.dev, it serves the SSL certificate with teamcity
...
| DNS Name |  Type  | Text |
|:-----|--------:|:------|
|  _acme-challenge.dman.dev. | TXT | Copy and paste from the output of `certbot`. |
...
Once verified, certbot writes the pub / priv pair to `/etc/letsencrypt/live/dman.dev`
